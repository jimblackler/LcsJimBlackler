...
char *LCS_Blackler4(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *lcs) {
  ...
  for (int primaryIndex = primaryLength - 1;
    ...
    while (nextBlocks[firstBlockIndex] <
        longestSequence - remainingCharacters - 1)
      nextBlocks[firstBlockIndex] = nextBlocks[nextBlocks[firstBlockIndex]];

    unsigned char chr = primary[primaryIndex];
    int probe = characterMap[chr];
    ...
    if (sequenceLength == 0)
      ...
      // secondary string.
      while (probe != -1 && probe >= basePosition)
        probe = linkedChars[probe];

      // Update the first appearance character map.
      characterMap[chr] = probe;
    }
...
//      }
      while (probe >= end) {
        if (probe <= basePosition - 1) {
          secondaryIndex = probe;
          break;
        }
        probe = linkedChars[probe];
      }
