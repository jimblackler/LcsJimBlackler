...
char *LCS_Blackler8(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *lcs) {
  ...
  int *nextBlocks = calloc(upperLimit, sizeof(int));
  int firstBlock = 0;
  ...
  for (int primaryIndex = primaryLength - 1;
    ...
    while (firstBlock < longestSequence - remainingCharacters - 1)
      firstBlock = nextBlocks[firstBlock];
    ...
    if (sequenceLength == 0) {
      ...
      // secondary string.
      while (secondaryIndex != -1 && secondaryIndex >= basePosition)
        secondaryIndex = linkedChars[secondaryIndex];

      // Update the first appearance character map.
      characterMap[chr] = secondaryIndex;
    }
    ...
    while (secondaryIndex != -1) {
      ...
      if (secondaryIndex < basePosition) {
        int nextBlock;

        if (sequenceLength == longestSequence) {
          nextBlock = longestSequence + 1;
        } else {
          nextBlock = nextBlocks[sequenceLength];
          if (nextBlock != sequenceLength + 1) {
            // Shortening the next block.
            nextBlocks[sequenceLength + 1] = nextBlock;
            nextBlock = sequenceLength + 1;
          }
        }

        if (sequenceLength == 0 ? secondaryIndex == secondaryLength - 1 :
            secondaryIndex == maxSecondaries[sequenceLength - 1] - 1) {
          // Extending a block.
          if (previousBlock == -1)
            firstBlock = nextBlock;
          else
            nextBlocks[previousBlock] = nextBlock;
        } else {
          // New block.
          nextBlocks[sequenceLength] = nextBlock;
        }
        ...
        // Move to next block.
        previousBlock = sequenceLength;  // <--- check this!!!!
        if (nextBlock > sequenceLength + 1)
          basePosition = maxSecondaries[nextBlock - 1];
        sequenceLength = nextBlock;
