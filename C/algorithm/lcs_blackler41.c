...
static void insert(Cell **start, Cell *cell) {
  if (!*start) {
    *start = cell;
    return;
  }
  Cell *next = *start;
  while (next->next && next->next->area > cell->area) {
    next = next->next;
  }
  next->next = cell;
}

static int maxRectangleGreedyCS2(const char *primary, const char *secondary,
    int *lengthAtPrimary, int *lengthAtSecondary,
    size_t primaryLength, size_t secondaryLength) {
  int length = 0;
  int x = 0;
  int y = 0;
  while (x < primaryLength && y < secondaryLength) {
    Cell *start = malloc(sizeof(Cell));
    start->x = x;
    start->y = y;
    start->area = (primaryLength - start->x - 1) * (secondaryLength - start->y - 1);
    Cell *largest = start;

    while (largest) {
      if (primary[largest->x] == secondary[largest->y]) {
        while(x < largest->x + 1) {
          lengthAtPrimary[x] = length;
          x++;
        }
        while(y < largest->y + 1) {
          lengthAtSecondary[y] = length;
          y++;
        }
        length++;
        break;
      }
      Cell *prevLargest = largest;
      largest = prevLargest->next;
      if (prevLargest->y == y) {
        Cell *append = malloc(sizeof(Cell));
        append->x = prevLargest->x + 1;
        append->y = prevLargest->y;
        append->area = (primaryLength - append->x - 1) * (secondaryLength - append->y - 1);
        insert(&largest, append);
      }

      {
        Cell *append = malloc(sizeof(Cell));
        append->x = prevLargest->x;
        append->y = prevLargest->y + 1;
        append->area = (primaryLength - append->x - 1) * (secondaryLength - append->y - 1);
        insert(&largest, append);
      }
    ...
    while (largest) {
      struct Cell *nextCell = largest->next;
      free(largest);
      largest = nextCell;
    }
  }
...
static int maxRectangleGreedyCS(const char *primary, const char *secondary,
    int *lengthAtPrimary, int *lengthAtSecondary,
    size_t primaryLength, size_t secondaryLength) {
  int cs = 0;
  int x = 0;
  ...
  while (x < primaryLength && y < secondaryLength) {
    ...
    size_t target = (primaryLength - x - 1) * (secondaryLength - y - 1);

    while (true) {

      for (int pass = 1; pass <= 2; pass++) {
        size_t maxValue = 0;
        size_t column0 = (primaryLength - x - 1) * (secondaryLength - y - 1);

        for (int y1 = y; y1 < secondaryLength; y1++) {
          if (target > column0)
            break;
          size_t columnDelta = secondaryLength - y1 - 1;
          size_t x1;
          if (columnDelta)
            x1 = (column0 - target + columnDelta - 1) / columnDelta + x;
          else
            x1 = x;

          size_t value = column0 - (x1 - x) * columnDelta;
          if (value > maxValue)
            maxValue = value;
          if (value == target) {
            if (primary[x1] == secondary[y1]) {
              while (x < x1) {
                lengthAtPrimary[x] = cs;
                x++;
              }
              while (y < y1) {
                lengthAtSecondary[y] = cs;
                y++;
              }
              cs++;
              lengthAtPrimary[x] = cs;
...
static char *LCS(const char *primary, const char *secondary,
  ...
  int lowerLimit = maxRectangleGreedyCS2(primary, secondary, lengthAtPrimary, lengthAtSecondary,
      primaryLength, secondaryLength);
...
char *LCS_Blackler41(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *length) {
