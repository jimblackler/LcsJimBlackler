...
//  long long *timeAtPrimaries = calloc(primaryLength, sizeof(long long));
...
//  int extends0 = 0;
//printf("\nprimaryLength - primaryIndex, longestSequence, secondaryLength - basePosition, sequenceLength\n");

  for (int primaryIndex = primaryLength - 1;
       primaryIndex >= 0 && longestSequence < upperLimit;
       primaryIndex--) {
    
//    timeAtPrimaries[primaryIndex] = getMicroseconds();
...
//      const char *ptr = &secondary[basePosition - 1];
//      const char *endPtr = &secondary[end];
//      while (ptr >= endPtr) {
//        if (*ptr == chr) {
//          secondaryIndex = ptr - secondary;
//          break;
//        }
//        ptr--;
//      }
  ...
  *lcs = longestSequence;
  ...
  for (int primaryIndex = 0; longestSequence2 < longestSequence;
    ...
    while (!baseSequence || baseSequence->secondaryIndex < secondaryLength - 1) {
      ...
      int start;
      if (baseSequence)
        start = baseSequence->secondaryIndex + 1;
      else
        start = 0;
      int end;
      if (sequenceLength == longestSequence2)
        end = maxSecondaries[longestSequence - longestSequence2 - 1];
      else
        end = sequences[sequenceLength]->secondaryIndex - 1;
...
char *LCS_Blackler21(const char *primary, const char *secondary,
