...
static int probabilisticCS(const char *primary, const char *secondary,
...
loop:
  for (int z = 0;; z++) {
    for (int y0 = 0; y0 <= z; y0++) {
      ...
      int b1 = b + y;
      if (a1 >= primaryLength) {
        if (b1 >= secondaryLength) {
          while (a < primaryLength) {
            lengthAtPrimary[a] = cs;
            a++;
          }
          while (b < secondaryLength) {
            lengthAtSecondary[b] = cs;
            b++;
          }
...
static int cleverGreedyCS(const char *primary, const char *secondary,
  ...
  while (a1 < primaryEnd && b1 < secondaryEnd) {
    if (*a == *b1) {
      ...
      if ((primaryEnd - a) * (secondaryEnd - b1)  < (primaryEnd - a1) * (secondaryEnd - b))
        a1++;
      else
        b1++;
    }
...
static int cleverGreedyCS2(const char *primary, const char *secondary,
                           int *lengthAtPrimary, int *lengthAtSecondary,
                           size_t primaryLength, size_t secondaryLength) {
...
static char *LCS(const char *primary, const char *secondary,
  ...
  int lowerLimit = probabilisticCS(primary, secondary, lengthAtPrimary, lengthAtSecondary,
                                   primaryLength, secondaryLength);
  
  int *lengthAtPrimary2 = malloc(primaryLength * sizeof(int));
  int *lengthAtSecondary2 = malloc(secondaryLength * sizeof(int));
  
  cleverGreedyCS2(primary, secondary, lengthAtPrimary2, lengthAtSecondary2,
                  primaryLength, secondaryLength);

  int *lengthAtPrimary3 = malloc(primaryLength * sizeof(int));
  int *lengthAtSecondary3 = malloc(secondaryLength * sizeof(int));
  
  cleverGreedyCS(primary, secondary, lengthAtPrimary3, lengthAtSecondary3,
                 primaryLength, secondaryLength);
  ...
  for (int primaryIndex = primaryLength - 1;
    ...
    int remainA2 = lengthAtPrimary2[primaryIndex];
    int remainA3 = lengthAtPrimary3[primaryIndex];
    int sequenceLength;
    int basePosition;
    ...
    while (basePosition > 0) {
      ...
      if (secondaryIndex <= maxSecondaries[sequenceLength]) {
        ...
        int remainB2 = lengthAtSecondary2[secondaryIndex];
        int minRemain2 = remainA2 < remainB2 ? remainA2 : remainB2;
        int potentialMax2 = sequenceLength + minRemain2;
        if (lowerLimit < potentialMax2) {
          lowerLimit = potentialMax2;
        }

        int remainB3 = lengthAtSecondary3[secondaryIndex];
        int minRemain3 = remainA3 < remainB3 ? remainA3 : remainB3;
        int potentialMax3 = sequenceLength + minRemain3;
        if (lowerLimit < potentialMax3) {
          lowerLimit = potentialMax3;
        }
  ...
  free(lengthAtPrimary2);
  free(lengthAtSecondary2);

  free(lengthAtPrimary3);
  free(lengthAtSecondary3);
...
char *LCS_Blackler38(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *length) {
