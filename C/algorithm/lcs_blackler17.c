...
static char *LCS(const char *primary, const char *secondary,
  ...
  assert (lowerLimit <= primaryLength);
  assert (lowerLimit <= secondaryLength);
  ...
  *lcs = 0;
  ...
  for (int primaryIndex = primaryLength - 1;
    ...
    int remainingCharacters = primaryIndex + 1;
    ...
    if (minimumSequence < longestSequence - remainingCharacters - 1)
      minimumSequence = longestSequence - remainingCharacters - 1;

    if (minimumSequence < lowerLimit - remainingCharacters - 1)
      minimumSequence = lowerLimit - remainingCharacters - 1;
    ...
    if (minimumSequence == 0) {
      sequenceLength = 0;
      baseSequence = -1;
      if (maxSecondaries[0] == primaryLength - 1)
        minimumSequence++;
    } else {
      while (minimumSequence < longestSequence &&
          maxSecondaries[minimumSequence] ==
              maxSecondaries[minimumSequence - 1] - 1)
        minimumSequence++;
      sequenceLength = minimumSequence;
      baseSequence = maxSecondaries[sequenceLength - 1];
      // Pre-scan to the first viable appearance of the character in the
      // secondary string.
      while (secondaryIndex != -1 && secondaryIndex >= baseSequence)
    ...
    while (secondaryIndex != -1) {
      ...
      while (sequenceLength < longestSequence &&
          secondaryIndex <= maxSecondaries[sequenceLength]) {
        baseSequence = maxSecondaries[sequenceLength];
        sequenceLength++;
      }
      
      if (secondaryIndex < lowerLimit - sequenceLength - 1)
        break;
      // Ensure that any base sequence can legitimately be extended
      if (baseSequence == -1 || secondaryIndex < baseSequence) {
  ...
  *lcs = longestSequence;
  ...
  for (int primaryIndex = 0; longestSequence2 < longestSequence;
    ...
    if (minimumSequence == 0) {
      ...
      while (minimumSequence < longestSequence2 &&
          sequences[minimumSequence]->secondaryIndex ==
              sequences[minimumSequence - 1]->secondaryIndex + 1)
        minimumSequence++;
      sequenceLength = minimumSequence;
    ...
    while (secondaryIndex != -1 && secondaryIndex <= c) {
      ...
      if (baseSequence == NULL ||
        ...
        if (!lastRecycledNode && activePool->nextSlot - activePool->firstSlot == activePool->size) {
  ...
  assert(longestSequence == longestSequence2);
...
char *LCS_Blackler17(const char *primary, const char *secondary,
    size_t primaryLength, size_t secondaryLength, size_t *length) {
