...
//  printf("%d %d\n", maxPairsRemain, primaryIndex + 1);
    ...
    int endIdx = firstIndices[chr + 1]; // what about the last char?
    ...
    while (basePosition > 0 && appIdx < endIdx) {
      ...
      if (appearances[appIdx] >= end) {
        int endPoint = endIdx - 1;
        while (true) {
          if (appIdx == endPoint) {
            if (appearances[appIdx] < basePosition)
              secondaryIndex = appearances[appIdx];
            appIdx++;
            break;
          }

          int midPoint = appIdx + (endPoint - appIdx) / 2;
...
char *LCS_Blackler5(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *length) {
