...
static int twoWayCS(const char *primary, const char *secondary,
    size_t primaryLength, size_t secondaryLength, int *sp, int *ct) {
...
static char *LCS(const char *primary, const char *secondary,
  ...
  int *ct = malloc(primaryLength * sizeof(int));
  int *sp = malloc(primaryLength * sizeof(int));
  int lowerLimit = twoWayCS(primary, secondary, primaryLength, secondaryLength, sp, ct);
  ...
  for (int primaryIndex = primaryLength - 1;
    ...
    if (longestSequence > 0 && sp[primaryIndex] < maxSecondaries[longestSequence - 1]) {
      int projected = longestSequence + ct[primaryIndex];

      if (lowerLimit < projected) {
//            printf("%d %d %d %d %d %d %d\n", primaryIndex, sp[primaryIndex - 1],  maxSecondaries[longestSequence - 1], ct[primaryIndex - 1], longestSequence, lowerLimit, projected);
        lowerLimit = projected;
      }
    }
...
char *LCS_Blackler27(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *length) {
