...
static int twoWayCS(const char *primary, const char *secondary,
    size_t primaryLength, size_t secondaryLength, int* lowerLimitsByPrimary,
    int* lowerLimitsBySecondary, int *lowerLimitsPrimaryPosition,
    int *lowerLimitsSecondaryPosition) {
  int cs = 0;
  {
    int _a = 0;
    int _b = 0;
    ...
    while (_a1 < primaryLength && _b1 < secondaryLength) {
      if (primary[_a] == secondary[_b1]) {
        cs++;
        lowerLimitsPrimaryPosition[_b1] = _a;
        lowerLimitsSecondaryPosition[_a] = _b1;
        lowerLimitsByPrimary[_a] = cs;
        lowerLimitsBySecondary[_b1] = cs;
        _a++;
        _b = _b1 + 1;
        _a1 = _a;
        _b1 = _b;
        ...
        cs++;
        lowerLimitsPrimaryPosition[_b] = _a1;
        lowerLimitsSecondaryPosition[_a1] = _b;
        lowerLimitsByPrimary[_a1] = cs;
        lowerLimitsBySecondary[_b] = cs;
        _b++;
        _a = _a1 + 1;
        _a1 = _a;
        _b1 = _b;
  ...
  if (cs == primaryLength || cs == secondaryLength) {
    ...
    for (int idx = 1; idx < primaryLength; idx++) {
      if (lowerLimitsSecondaryPosition[idx - 1] < sp2[idx]) {
        int t = lowerLimitsByPrimary[idx - 1] + ct2[idx];
        if (t >= m) {
          m = t;
        }
      }
    }
    if (lowerLimitsByPrimary[primaryLength - 1] > m)
      m = lowerLimitsByPrimary[primaryLength - 1];
...
static char *LCS(const char *primary, const char *secondary,
  ...
  int *lowerLimitsByPrimary = malloc(primaryLength * sizeof(int));
  int *lowerLimitsBySecondary = malloc(secondaryLength * sizeof(int));
  int *lowerLimitsPrimaryPosition = malloc(secondaryLength * sizeof(int));
  int *lowerLimitsSecondaryPosition = malloc(primaryLength * sizeof(int));
  int lowerLimit = twoWayCS(primary, secondary, primaryLength, secondaryLength,
      lowerLimitsByPrimary, lowerLimitsBySecondary, lowerLimitsPrimaryPosition,
      lowerLimitsSecondaryPosition);
  ...
  for (int primaryIndex = primaryLength - 1;
    ...
    if (!longestSequence || lowerLimitsSecondaryPosition[primaryIndex] < maxSecondaries[longestSequence - 1]) {
      int prospect = longestSequence + lowerLimitsByPrimary[primaryIndex];
      if (prospect > lowerLimit)
        lowerLimit = prospect;
    }
    
    if (false && longestSequence) { // !!!
      int x = maxSecondaries[longestSequence - 1];
      if (lowerLimitsPrimaryPosition[x] < primaryIndex) {
        int prospect = longestSequence + lowerLimitsBySecondary[x];
        if (prospect > lowerLimit) {
//              printf("%d %d %d %d\n ", primaryIndex, x, lowerLimitsPrimaryPosition[x], lowerLimitsBySecondary[x]);
          lowerLimit = prospect;
        }
      }
    }
  ...
  free(lowerLimitsByPrimary);
  free(lowerLimitsBySecondary);
  free(lowerLimitsPrimaryPosition);
  free(lowerLimitsSecondaryPosition);
...
char *LCS_Blackler23(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *length) {
