...
static char *LCS(const char *primary, const char *secondary,
  ...
  int *secondaryCounts = calloc(numberCharacters, sizeof(int));
  ...
  int *primaryCounts = calloc(numberCharacters, sizeof(int));
  for (int idx = 0; idx != primaryLength; idx++) {
    unsigned char chr = primary[idx];
    primaryCounts[chr]++;
  }

  int upperLimit = 0;
  for (int chr = 0; chr != numberCharacters; chr++) {
    int min = primaryCounts[chr];
    if (secondaryCounts[chr] < min)
      min = secondaryCounts[chr];
    upperLimit += min;
  }

//printf("%d %zu\n", upperLimit, primaryLength);
  ...
  *lcs = 0;
  ...
  int maxPairsRemain = upperLimit;

  int doneSecondaryTo = secondaryLength;

  int longestSequence = 0;  // The longest known sequence.
  maxSecondaries[longestSequence] = -1;

//printf("primaryLength - primaryIndex, primaryIndex, maxPairsRemain, longestSequence, secondaryLength - basePosition, sequenceLength\n");
...
//  printf("%d %d\n", maxPairsRemain, primaryIndex + 1);
    ...
    int remainingCharacters = maxPairsRemain;//primaryIndex + 1; // maxRemainPrimary;
    ...
    while (basePosition < doneSecondaryTo) {
      --doneSecondaryTo;
      unsigned char chr2 = secondary[doneSecondaryTo];

      secondaryCounts[chr2]--;
      if (secondaryCounts[chr2] < primaryCounts[chr2])
        maxPairsRemain--;
    }
...
//    printf("primary: %lu basePosition : %lu (to %lu) %d\n", primaryLength - primaryIndex, secondaryLength - basePosition, secondaryLength, sequenceLength);

//    printf("%lu, %d, %d, %d, %lu, %d\n", primaryLength - primaryIndex, primaryIndex, maxPairsRemain, longestSequence, secondaryLength - basePosition, sequenceLength);
...
//          }
    ...
    primaryCounts[chr]--;
    if (primaryCounts[chr] < secondaryCounts[chr])
      maxPairsRemain--;
  }
...
//printf("\n{%f}\n", (float)lowerLimit / longestSequence);
//printf("\n(%f)\n", (float)cleverGreedyCS(primary, secondary, primaryLength, secondaryLength) / longestSequence);
...
char *LCS_Blackler15(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *length) {
