...
static int cleverGreedyCS(const char *primary, const char *secondary,
  ...
  while (a1 < primary + primaryLength && b1 < secondary + secondaryLength) {
    if (*a == *b1) {
      a++;
      b = b1 + 1;
      a1 = a;
...
static int reallyCleverGreedyCS(const char *primary, const char *secondary,
                                size_t primaryLength, size_t secondaryLength) {
  
  int numberCharacters = 1 << sizeof(char) * 8;
  size_t characterMapSize = sizeof(int) * numberCharacters;
  int* primaryAppearance = malloc(characterMapSize);
  memset(primaryAppearance, -1, characterMapSize);
  int* secondaryAppearance = malloc(characterMapSize);
  memset(secondaryAppearance, -1, characterMapSize);
  ...
  while (a < primaryLength && b < secondaryLength) {
    unsigned char aChar = primary[a];
    unsigned char bChar = secondary[b];
    primaryAppearance[aChar] = a;
    secondaryAppearance[bChar] = b;
    int atA = primaryAppearance[bChar];
    int atB = secondaryAppearance[aChar];
    int aDelta = atA - lastA;
    int bDelta = atB - lastB;
    if (aDelta > 0 && (bDelta <= 0 || aDelta < bDelta)) {
      a = atA;
      lastA = a;
      lastB = b;
      cs++;
  ...
  free(primaryAppearance);
  free(secondaryAppearance);
  
  return cs;
}

static int fastProbabilisticLCS(const char *primary, const char *secondary,
                                size_t primaryLength, size_t secondaryLength) {
...
static char *LCS(const char *primary, const char *secondary,
  ...
  //  int v1 = cleverGreedyCS(primary, secondary, primaryLength, secondaryLength);
  //  int v2 = reallyCleverGreedyCS(primary, secondary, primaryLength, secondaryLength);
  int v3 = fastProbabilisticLCS(primary, secondary, primaryLength, secondaryLength);
  int lowerLimit = 0;
  //  if (v1 > lowerLimit)
  //    lowerLimit = v1;
  //  if (v2 > lowerLimit)
  ...
  int *minSecondaries = calloc(upperLimit + 1, sizeof(int));
    ...
    while (basePosition > 0) {
      ...
      if (secondaryIndex == -1) {
        ...
        if (sequenceLength == longestSequence) {
          ...
          minSecondaries[sequenceLength] = secondaryIndex;
          longestSequence++;
  ...
  for (int primaryIndex = 0; longestSequence2 < longestSequence;
    ...
    while (secondaryIndex != -1 && secondaryIndex <= c) {
      ...
      if (baseSequence == NULL ||
        ...
        // woah
        if (secondaryIndex < minSecondaries[longestSequence - sequenceLength - 1]) {
          minimumSequence = sequenceLength + 1;
        }
...
char *LCS_Blackler31(const char *primary, const char *secondary, size_t primaryLength, size_t secondaryLength, size_t *length) {
